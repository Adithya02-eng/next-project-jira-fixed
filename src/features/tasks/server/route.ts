...
.delete(
    ...
    async(c)=>{
        ...
        if(!member)
        {
            return c.json({error:"UnAuthrozied"},401);
        }
...
.get(
  ...
  assigneeIds.filter((id): id is string => !!id && typeof id === 'string' && id.trim() !== '')
)];

        const memberQueries = uniqueAssigneeIds.map(id => Query.equal("$id", id));
        console.log("Smt",assigneeIds,"Lenght is",assigneeIds.length);
...
.post(
    ...
    async(c)=>{
        ...
        {
            return c.json({error:"All Tasks must Belong tom Same Workspace"});
        }
